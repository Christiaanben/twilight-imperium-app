...
export function drawHex(x: number, y: number, size: number, canvas: CanvasRenderingContext2D, orientation: HexOrientation = 'pointy') {
    canvas.beginPath()
    let func1 = Math.sin
    let func2 = Math.cos
    size *= .98
    ...
    for (let side = 0; side < 7; side++)
        canvas.lineTo(
            x + size * func1(side * Math.PI / 3),
            y + size * func2(side * Math.PI / 3))
